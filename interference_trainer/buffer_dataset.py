...
class BufferDataset(Dataset):
    ...
    def __init__(self, parquet_files: Union[str, List[str]], tokenizer, config):
        ...
        max_length = config.get("max_length", 9216)
    ...
    def __getitem__(self, item):
        ...
        if isinstance(row_dict["reward_model"]["ground_truth"], str):
            row_dict["reward_model"]["ground_truth"] = json.loads(row_dict['reward_model']["ground_truth"])

        if "metadata" in row_dict['reward_model']:
            row_dict['reward_model']['metadata']= json.loads(row_dict['reward_model']['metadata'])
            if "variables" in row_dict['reward_model']['metadata']:
                if isinstance(row_dict['reward_model']['metadata']['variables'], dict):
                    keys = row_dict['reward_model']['metadata']['variables'].keys()
                    for k in keys:
                        if "fraction" in k:
                            row_dict['reward_model']['metadata']['variables'][k] = Fraction(row_dict['reward_model']['metadata']['variables'][k])
        return {
            "prompts": prompt_ids,
            "index": index,
            ...
            # "response_mask": attention_mask[-response_ids.shape[0]:],
