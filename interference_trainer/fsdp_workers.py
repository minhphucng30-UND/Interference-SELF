...
class ClipActorRolloutRefWorker(Worker):
    """
    ...
    def init_model(self):
        from actor import DataParallelClipPPOActor
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelClipPPOActor(config=self.config.actor, actor_module=self.actor_module_fsdp, actor_optimizer=self.actor_optimizer)
        ...
        if self._is_ref:
            ...
            self.ref_policy = BufferDataParallelPPOActor(config=self.config.ref, actor_module=self.ref_module_fsdp)
